(function($) {
	$.fn.slide = function(options) {
		...
		function toLast(move) {
			switch (move) {
				case 'last':
					el.find('.slides-item:first').css('margin-left', (el.children().length * 100) + "%");
					el.changeSlide(settings, -el.children().length * 100);
					el.find('.slides-item:first').imgMoveSetting('right');
					setTimeout(function() {
						el.css('left', '0%');
						el.find('.slides-item:first').css('margin-left', '0');
						el.find('.slides-item:first').addClass('active').siblings().removeClass('active');
						changeNavList();
					}, settings.slideSpeed + 50);
					break;
				case 'first':
					el.find('.slides-item:last').css('margin-left', -(el.children().length * 100) + "%");
					el.changeSlide(settings, 100);
					el.find('.slides-item:last').imgMoveSetting('left');
					setTimeout(function() {
						el.css('left', -((el.children().length - 1) * 100) + "%");
						el.find('.slides-item:last').css('margin-left', '0');
						el.find('.slides-item:last').addClass('active').siblings().removeClass('active');
						changeNavList();
					}, settings.slideSpeed + 50);
					break;
			}
